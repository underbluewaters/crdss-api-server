...
app.get("/:v/mvt/:z/:x/:y", async (c, next) => {
  ...
  startTime(c, "db");
  const result = await all<{ id: string; count: number, all_count: number }>(query, values);
  endTime(c, "db");
  if (result.length === 0) {
    if (format === "geojson") {
      return c.json({
  ...
  for (const property of properties) {
    ...
    if (property.type === "number") {
      ...
      const histogramRows = await all<{
        bin: number; bin_start: number; bin_end: number; count: number;
      }>(`
        ${cte}
        SELECT
          bin::INTEGER AS bin,
      ...
      const result = await get<{ min: number, max: number, avg: number, count: number }>(query);
